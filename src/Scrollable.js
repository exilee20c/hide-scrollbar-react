...
class Scrollable extends Component {
  constructor(props) {
    ...
    this.mouseScrollStart = this.mouseScrollStart.bind(this);
    this.mouseScrollEnd = this.mouseScrollEnd.bind(this);
  ...
  mouseScrollStart ( event ) {
    ...
    if ( this.refs.scrollbar_inner === document.elementFromPoint(event.clientX, event.clientY) ) {
      this.setState({
        ...
        thumb_press_origin_y_of: event.clientY - this.refs.scrollbar_inner.getBoundingClientRect().top,
      });
    }
    ...
    // 바 클릭시 --> 썸의 가운데 위치 기억 후 드래그 대응
      ...
      this.setState({
        ...
        thumb_press_origin_x: event.clientX,
        thumb_press_origin_y_of: this.refs.scrollbar_inner.getBoundingClientRect().height / 2,
  ...
  mouseMove ( event ) {
    if ( event.buttons && this.state.is_thumb_pressed ) {
      if ( this.refs ) {
        let the_scale_amt = this.refs.scrollbar_outer.getBoundingClientRect().height / this.refs.scrollbar_inner.getBoundingClientRect().height;

        if ( this.refs.scrollbar_outer && this.refs.scrollbar_inner && this.refs.scrollable_outer ) {
          ...
          if ( Math.abs(event.clientX - this.state.thumb_press_origin_x) > 100 ) {
            ...
            this.refs.scrollable_outer.scrollTop = the_scale_amt * ( event.clientY - this.state.thumb_press_origin_y_of - this.refs.scrollbar_outer.getBoundingClientRect().top );
          }
        }
      }
    }
  ...
  render() {
    ...
    return (
      <section className="bp-scrollable-wrapper" style={this.props.style}>
        <div className="bp-scrollable-container" style={this.props.inner_style} ref="scrollable_outer" onMouseEnter={this.scrollChanged} onScroll={this.scrollChanged}>
          <div className="bp-scrollable-content" ref="scrollable_inner">
            {this.props.children}
          </div>
        </div>

          <div className={`bp-scrollable-scrollbar-wrapper${scrollable_inner_height > scrollable_outer_height ? this.state.is_appear ? ' appear' : '' : ' disappear'}`} ref="scrollbar_outer"
               onMouseDown={this.mouseScrollStart}          
          >
            <div className="bp-scrollable-scrollbar-thumb" ref="scrollbar_inner"
              style={{
                height: `${scrollable_inner_height > scrollable_outer_height ? (scrollable_outer_height / scrollable_inner_height * scrollbar_outer_height) : scrollbar_outer_height}px`,
                top: `${(scrollbar_outer_height - scrollbar_inner_height) * (this.state.scroll_top / (scrollable_inner_height - scrollable_outer_height))}px`,
              }}
              onMouseDown={this.mouseScrollStart}
            />
          </div>
